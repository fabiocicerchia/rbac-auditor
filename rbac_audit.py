...
def dangling_binding_findings(snap):
    sas = {(sa["metadata"]["namespace"], sa["metadata"]["name"]) for sa in snap["serviceaccounts"]}
    for kind in ("rolebindings", "clusterrolebindings"):
        for b in snap[kind]:
            for s in b.get("subjects") or []:
                if s.get("kind") == "ServiceAccount":
                    key = (s.get("namespace", b["metadata"].get("namespace", "")), s["name"])
                    if key not in sas:
...
def report(snap):
    ...
    for title, findings in sections:
        ...
        total += len(findings)
    print(f"## Inventory\n")
    for kind in ("roles", "clusterroles", "rolebindings", "clusterrolebindings", "serviceaccounts"):
...
def who_can(verb, resource, snap):
    def rule_matches(rule):
        ...
        return ("*" in verbs or verb in verbs) and ("*" in resources or resource in resources)

    granting = set()
    for kind in ("roles", "clusterroles"):
        for role in snap[kind]:
            if any(rule_matches(r) for r in role.get("rules") or []):
                granting.add((kind[:-1].replace("role", "Role").replace("clusterRole", "ClusterRole"), name(role)))
    for kind in ("rolebindings", "clusterrolebindings"):
        for b in snap[kind]:
            ref = b.get("roleRef", {})
            ns_name = f'{b["metadata"].get("namespace", "")}/{ref.get("name", "")}'.lstrip("/")
            if any(n_ in (ref.get("name"), ns_name) for _, n_ in granting):
                for s in b.get("subjects") or []:
                    print(f'{s.get("kind")} {s.get("namespace", "")}/{s.get("name")}'.replace(" /", " "))
